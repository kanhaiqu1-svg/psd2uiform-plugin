...
namespace UGF.EditorTools.Psd2UGUI
{
    internal static class CliCommandResolver
    {
        ...
        internal static bool TryResolve(string commandName, string commandArguments, string promptPath, out LaunchSpec launchSpec, out string error)
        {
            launchSpec = default;
            error = null;
                ...
                switch (extension)
                {
                    ...
                    case ".bat":
                        launchSpec = new LaunchSpec(
                            ...
                            $"/d /s /c \"\"{resolvedPath}\" {commandArguments} < \"{promptPath}\"\"",
                            false);
                        return true;
                    ...
                    case ".ps1":
                        launchSpec = new LaunchSpec(
                            "powershell.exe",
                            $"-NoProfile -ExecutionPolicy Bypass -Command \"Get-Content -Raw -LiteralPath '{EscapeForSingleQuotedPowerShell(promptPath)}' | & '{EscapeForSingleQuotedPowerShell(resolvedPath)}' {commandArguments}\"",
                            false);
                        return true;
                }
            }

            launchSpec = new LaunchSpec(resolvedPath, commandArguments, true);
            return true;
        }
        ...
        private static string[] BuildCandidateFileNames(string commandName)
        {
            ...
            return new[] { commandName + ".cmd", commandName + ".exe", commandName + ".bat", commandName + ".ps1", commandName };
        }
        ...
        private static IEnumerable<string> EnumerateSearchDirectories()
        {
            ...
            foreach (var pathDir in SplitPath(Environment.GetEnvironmentVariable("PATH")))
            {
                if (seen.Add(pathDir))
                {
            ...
            if (IsWindows())
            {
                foreach (var pathDir in SplitPath(TryGetEnvironmentVariable("PATH", EnvironmentVariableTarget.User)))
                {
                    if (seen.Add(pathDir))
                    {
                        yield return pathDir;
                ...
                foreach (var pathDir in SplitPath(TryGetEnvironmentVariable("PATH", EnvironmentVariableTarget.Machine)))
                {
                    if (seen.Add(pathDir))
                    {
                        yield return pathDir;
